...
(function () {
  ...
  function humanAuthError(error) {
    ...
    if (
      ...
      error?.constructor?.name === 'AuthRetryableFetchError' ||
      /AuthRetryableFetchError/i.test(String(error))
    ) {
      return 'Service error while sending magic link.';
    }
...
const authMode = form.dataset.authMode || document.body.dataset.page || page() || 'login';
...
if (isSignup) {
  signInOptions.data = {
    ...
    account_type: profile.accountType || 'wellness-seeker',
    account_role: profile.role || 'seeker',
    role: profile.role || 'seeker',
    marketing_opt_in: Boolean(profile.marketingOptIn),
    newsletter_opt_in: Boolean(profile.marketingOptIn)
  };
}
...
const { error } = await sb.auth.signInWithOtp({
  ...
  async function getActiveSessionAndUser() {
    ...
    // No one is logged in yet. This is normal on /login/ and /signup/.
  ...
  document.addEventListener('DOMContentLoaded', function () {
    if (page() === 'login' || page() === 'signup') initLoginPage();
