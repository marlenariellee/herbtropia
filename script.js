...
function loadRemoteDataJSONP() {
  ...
  window[callbackName] = function(payload) {
    try {
      if (payload && payload.ok) {
        state.practitioners = mergeById(state.practitioners, payload.practitioners || []).filter(isLive);
        state.events = mergeById(state.events, payload.events || []).filter(isLive);
        saveLocalData();
        renderCurrentPage();
      }
    } finally {
  ...
  script.onerror = function() {
    console.warn('Could not load remote Herbtropia directory data. Showing local data.');
    delete window[callbackName];
    script.remove();
  };
...
function renderPractitionerCard(p) {
  ...
  const tags = mergeTagValues(p.wellnessFocus, p.serviceTags).slice(0, 5).map(tag => `<span class="tag">${escapeHTML(displayLabels(tag, { ...CATEGORY_LABELS, ...FORMAT_LABELS, ...LOCATION_LABELS }) || labelizeSlug(tag))}</span>`).join('');
  const image = p.photoUrl ? `<img src="${escapeHTML(normalizeUrl(p.photoUrl))}" alt="${escapeHTML(p.businessName || p.name)}">` : escapeHTML(getInitials(p.businessName || p.name));
  return `<article class="profile-card">
    ...
    <p class="card-desc">${escapeHTML(p.bio || p.services || 'Profile details coming soon.').slice(0, 180)}${String(p.bio || p.services || '').length > 180 ? '…' : ''}</p>
    <div class="tag-row">${tags}</div>
    <div class="card-actions"><button class="card-action secondary" type="button" data-open-practitioner="${escapeHTML(p.id)}">View Profile</button>${p.bookingLink ? `<a class="card-action" href="${escapeHTML(normalizeUrl(p.bookingLink))}" target="_blank" rel="noopener">Book</a>` : ''}</div>
  </article>`;
}
...
function openPractitionerModal(id) {
  ...
  const profileImage = p.photoUrl ? `<div class="modal-image-wrap profile-photo"><img src="${escapeHTML(normalizeUrl(p.photoUrl))}" alt="${escapeHTML(p.businessName || p.name)}"></div>` : '';
...
function renderEventCard(event) {
  ...
  const location = formatLocationDisplay(event) || format;
  const tags = mergeTagValues(event.audience, event.eventFocus).slice(0, 5).map(tag => `<span class="tag">${escapeHTML(displayLabels(tag, { ...CATEGORY_LABELS, ...FORMAT_LABELS, ...LOCATION_LABELS, ...COST_LABELS }) || labelizeSlug(tag))}</span>`).join('');
  ...
  return `<article class="event-card">
    ...
    <p class="card-desc">${escapeHTML(event.description || 'Event details coming soon.').slice(0, 190)}${String(event.description || '').length > 190 ? '…' : ''}</p>
    <div class="tag-row">${tags}</div>
    <div class="card-actions"><button class="card-action secondary" type="button" data-open-event="${escapeHTML(event.id)}">View Event</button>${event.eventLink ? `<a class="card-action" href="${escapeHTML(normalizeUrl(event.eventLink))}" target="_blank" rel="noopener">RSVP</a>` : ''}</div>
  </article>`;
}
...
function openEventModal(id) {
  ...
  const flyerImage = event.imageUrl ? `<div class="modal-image-wrap event-flyer"><img src="${escapeHTML(normalizeUrl(event.imageUrl))}" alt="${escapeHTML(event.eventName)} flyer"></div>` : '';
  content.innerHTML = `<div class="modal-head">
      <div class="card-badge">${escapeHTML(category)}</div>
