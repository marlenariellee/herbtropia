...
function loadRemoteDataJSONP() {
  ...
  const useFallback = (reason) => {
    console.warn('Could not load remote Herbtropia directory data.', reason || '');
    const local = readLocalData();
    state.practitioners = Array.isArray(local.practitioners) ? local.practitioners.filter(isLive) : [];
  ...
  window[callbackName] = function(payload) {
    ...
    try {
      if (payload && payload.ok) {
        // Replace with current approved Google Sheet data.
  ...
  // Prevent the directory from appearing stuck forever if Apps Script returns invalid JS,
  // an HTML error page, or an old deployment that never calls the callback.
  setTimeout(() => {
    if (completed) return;
...
function renderPractitionerCard(p) {
  ...
  const image = p.photoUrl
    ...
    : `<span>${escapeHTML(getInitials(p.businessName || p.name))}</span>`;

  return `<article class="profile-card horizontal-card">
    ...
    <div class="card-main">
      ...
      <div class="card-meta">${escapeHTML(['📍 ' + location, format].filter(Boolean).join(' • '))}</div>
...
document.addEventListener('click', function(e) {
  ...
  if (
    ...
    href.includes('/practitioner-signup/') ||
