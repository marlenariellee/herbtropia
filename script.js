...
function openPractitionerModal(id) {
  ...
  <button class="modal-profile-photo" type="button" data-open-image="${escapeHTML(normalizeUrl(p.photoUrl))}" aria-label="View ${escapeHTML(p.businessName || p.name)} profile photo larger">
    <img src="${escapeHTML(normalizeUrl(p.photoUrl))}" alt="${escapeHTML(p.businessName || p.name)} profile photo">
  </button>
` : '';
...
function renderEventCard(event) {
  ...
  return `<article class="event-card ${hasFlyer ? 'has-flyer' : 'no-flyer'}">
