...
const EDUCATION_TOPIC_LABELS = {
  herbal: 'Herbal Medicine', 'herbal-medicine': 'Herbal Medicine',
  naturopathic: 'Naturopathic Medicine', 'naturopathic-medicine': 'Naturopathic Medicine',
  'nervous-system': 'Nervous System',
  somatic: 'Somatic Wellness', 'somatic-wellness': 'Somatic Wellness',
  functional: 'Functional Medicine', 'functional-medicine': 'Functional Medicine',
  nutrition: 'Nutrition',
  'skin-body': 'Skin + Body', 'beauty-wellness': 'Beauty + Wellness',
  spiritual: 'Spiritual Wellness', 'spiritual-wellness': 'Spiritual Wellness',
  movement: 'Movement',
  community: 'Community Wellness', 'community-wellness': 'Community Wellness',
  other: 'Other'
};
...
// ============================================
// Cleaned launch version:
// - Mobile nav initializes after the DOM loads.
// - Works on nested pages like /directory/, /events/, and /education/.
// - Keeps accessibility attributes synced with the visual menu state.
let menuOpen = false;

function setHamburgerIcon(open, toggleButton = document.getElementById('menuToggle')) {
...
function setMobileMenu(open) {
  ...
  setHamburgerIcon(menuOpen, menuToggle);
}
...
function initMobileNav() {
  ...
  if (menuToggle.dataset.navReady === 'true') {
    setMobileMenu(false);
    return;
  }
...
// Expose the backend bridge for account/Supabase scripts that need to submit
// optional newsletter/account welcome payloads without duplicating Apps Script logic.
window.HERBTROPIA_CONFIG = HERBTROPIA_CONFIG;
...
function openModal(modal) {
  ...
  document.body.style.overflow = 'hidden';
  modal._trapFocusHandler = (event) => trapModalFocus(event, modal);
  document.addEventListener('keydown', modal._trapFocusHandler);
  const focusTarget = modal.querySelector('[data-close-modal], .modal-close, h2, [tabindex]');
  if (focusTarget) {
    if (!focusTarget.hasAttribute('tabindex') && !/^(A|BUTTON|INPUT|SELECT|TEXTAREA)$/i.test(focusTarget.tagName)) focusTarget.setAttribute('tabindex', '-1');
...
function closeModal(modal) {
  ...
  document.body.style.overflow = '';
  if (modal._trapFocusHandler) document.removeEventListener('keydown', modal._trapFocusHandler);
...
function getEffectivePage() {
  ...
  if (!explicit || explicit === 'home') {
    return routeMap[path] || explicit || 'home';
  }
  return explicit;
}

async function updateAuthNavLabels() {
  const authLinks = document.querySelectorAll('[data-auth-nav-link]');
  const createLinks = document.querySelectorAll('[data-auth-create-link], [data-logged-out-only]');

  let signedIn = false;

  try {
    if (window.HerbtropiaSupabase && typeof window.HerbtropiaSupabase.getUser === 'function') {
      signedIn = Boolean(await window.HerbtropiaSupabase.getUser());
    }
  } catch (error) {
    signedIn = false;
  }

  /* Fallback: check Supabase localStorage session */
  if (!signedIn) {
    try {
      ...
      if (authKey) {
        ...
        const expiresAt = sessionData?.expires_at || sessionData?.currentSession?.expires_at;
  ...
  document.body.classList.toggle('user-is-logged-in', signedIn);
  document.body.classList.toggle('user-is-logged-out', !signedIn);

  authLinks.forEach((link) => {
    if (signedIn) {
      link.textContent = 'My Account';
      link.setAttribute('href', '/account/');
      link.setAttribute('aria-label', 'Go to my Herbtropia account');
    } else {
      link.textContent = 'Log In';
      link.setAttribute('href', '/login/');
      link.setAttribute('aria-label', 'Log in to Herbtropia');
    }
  });

  createLinks.forEach((link) => {
    link.hidden = signedIn;
    link.style.display = signedIn ? 'none' : '';
    link.setAttribute('aria-hidden', signedIn ? 'true' : 'false');
  });
}

window.HerbtropiaUpdateAuthNavState = updateAuthNavLabels;
...
// ============================================
// SPRINT PATCH — AUTH NAV LABELS + EVENTS CALENDAR VIEW
// Added for intuitive sign up/log in and clickable calendar browsing.
// ============================================

(function () {
  // Keep public nav intuitive. If Supabase is available, swap to My Account after login.
  window.updateAuthNavLabels = async function updateAuthNavLabels() {
    const links = document.querySelectorAll('[data-auth-nav-link]');
    ...
    links.forEach(link => {
      link.textContent = signedIn ? 'My Account' : 'Sign Up / Log In';
      link.setAttribute('href', signedIn ? '/account/' : '/login/');
      link.setAttribute('aria-label', signedIn ? 'Open my Herbtropia account' : 'Sign up or log in to Herbtropia');
    });
  };
  ...
  function getCalendarBaseDate() {
    if (calendarMonthCursor) return calendarMonthCursor;

    const today = new Date();
    const eventDates = (state.events || [])
      .map(getEventDateKey)
      .filter(Boolean)
      .sort();

    if (eventDates.length) {
      const [year, month] = eventDates[0].split('-').map(Number);
      calendarMonthCursor = new Date(year, month - 1, 1);
    } else {
      calendarMonthCursor = new Date(today.getFullYear(), today.getMonth(), 1);
    }

    return calendarMonthCursor;
  }

  function getEventsForCalendarBaseFilters() {
    const matcher = window.__herbtropiaOriginalEventMatches || eventMatches;
    return (state.events || []).filter(event => matcher(event));
  }

  function renderEventCalendar() {
    const calendar = document.getElementById('eventCalendar');
    const label = document.getElementById('eventCalendarMonthLabel');
    const selectedLabel = document.getElementById('eventCalendarSelectedLabel');
    if (!calendar || !label) return;

    const monthDate = getCalendarBaseDate();
    const year = monthDate.getFullYear();
    const month = monthDate.getMonth();
    label.textContent = getCalendarMonthLabel(monthDate);

    const baseFilteredEvents = getEventsForCalendarBaseFilters();
    const eventsByDate = baseFilteredEvents.reduce((map, event) => {
    ...
    for (let i = 0; i < startOffset; i += 1) {
      cells += '<div class="event-calendar-cell event-calendar-cell-empty" aria-hidden="true"></div>';
    }

    for (let day = 1; day <= daysInMonth; day += 1) {
      const key = `${year}-${padCalendarNumber(month + 1)}-${padCalendarNumber(day)}`;
      const dayEvents = eventsByDate[key] || [];
      const hasEvents = dayEvents.length > 0;
      const isSelected = selectedCalendarDate === key;
      const isToday = todayKey === key;
      const eventText = hasEvents ? `${dayEvents.length} event${dayEvents.length === 1 ? '' : 's'}` : 'No events';

      cells += `
        <button class="event-calendar-cell ${hasEvents ? 'has-events' : ''} ${isSelected ? 'selected' : ''} ${isToday ? 'today' : ''}" type="button" data-calendar-date="${key}" ${hasEvents ? '' : 'aria-disabled="true"'} aria-label="${day}, ${eventText}">
          <span class="event-calendar-day">${day}</span>
          <span class="event-calendar-count">${hasEvents ? eventText : ''}</span>
        </button>
      `;
    }

    calendar.innerHTML = `
      <div class="event-calendar-weekdays" aria-hidden="true">
        <span>Sun</span><span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span>
      </div>
      <div class="event-calendar-grid">${cells}</div>
    `;

    if (selectedLabel) {
      selectedLabel.textContent = selectedCalendarDate
        ? `Showing events for ${formatDate(selectedCalendarDate)}`
        : 'Click a date to filter the list below.';
    }
    ...
    calendar.querySelectorAll('[data-calendar-date]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        renderEvents();
        const resultsArea = document.getElementById('eventGrid');
  ...
  function initEventCalendarControls() {
    const calendarWrap = document.querySelector('[data-event-calendar-wrap]');
    if (!calendarWrap || calendarWrap.dataset.calendarReady === 'true') return;

    calendarWrap.dataset.calendarReady = 'true';

    const previous = document.getElementById('eventCalendarPrev');
    const next = document.getElementById('eventCalendarNext');
    const clear = document.getElementById('eventCalendarClear');

    if (previous) {
      previous.addEventListener('click', () => {
        const date = getCalendarBaseDate();
        calendarMonthCursor = new Date(date.getFullYear(), date.getMonth() - 1, 1);
        renderEventCalendar();
      });
    }
  ...
  // Save originals, then add date-filter behavior.
  window.__herbtropiaOriginalEventMatches = window.__herbtropiaOriginalEventMatches || eventMatches;
  const originalEventMatches = window.__herbtropiaOriginalEventMatches;

  eventMatches = function patchedEventMatches(event) {
    if (!originalEventMatches(event)) return false;
    if (!selectedCalendarDate) return true;
    return getEventDateKey(event) === selectedCalendarDate;
  };

  const originalRenderEvents = renderEvents;
  renderEvents = function patchedRenderEvents() {
    originalRenderEvents();
    renderEventCalendar();
  };

  const originalInitEventsPage = initEventsPage;
  initEventsPage = function patchedInitEventsPage() {
    originalInitEventsPage();
