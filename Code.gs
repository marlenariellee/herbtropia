...
function doPost(e) {
  try {
    ...
    // Route update requests before practitioner/event routing.
    if (submissionType === 'update-request') {
      return handleUpdateRequest_(data);
    }

    // IMPORTANT: do not default unknown submissions into Practitioners.
    // This prevents blank rows from being created when a form has the wrong type.
    if (submissionType !== 'practitioner' && submissionType !== 'event') {
      return output_({
        ok: false,
    ...
    if (type === 'event' && typeof hbSendInPersonReviewInterestIfNeeded_ === 'function') {
...
function doGet(e) {
  ...
  try {
    // Do NOT run setupSheets_() on public directory loads.
    // This endpoint should read approved rows only. Run runSetupSheets manually after code changes.
    payload = {
      ok: true,
...
function handleStatusEdit_(e) {
  ...
  // Case 1: Status changed to Approved or Rejected.
  ...
  // Case 2: Email address changed.
  // Do not resend past emails automatically. Just record the update so all future emails use the current row's email value.
...
function saveUpdateRequestImage_(data) {
  ...
  const isEventUpdate = String(data.listingType || '').toLowerCase() === 'event';
  ...
  const originalName = sanitizeFileName_(data[fileNameKey] || (isEventUpdate ? 'event-update-photo.jpg' : 'listing-update-photo.jpg'));
...
function hbBuildEmailParams_(data) {
  ...
  return {
    ...
    practitionerSignupUrl: `${websiteUrl}/practitioner-signup/`,
